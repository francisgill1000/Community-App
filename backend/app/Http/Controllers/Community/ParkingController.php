...
class ParkingController extends Controller
{
    ...
    {
        return Parking::orderBy('id', 'desc')->with(["floor", "vehicle"])->paginate(request("per_page") ?? 10);
    }
     ...
     * @return \Illuminate\Http\Response
     */
    public function floorByCategory($category)
    {
        return Parking::where('category', $category)
            ->distinct()
            ->join('floors', 'parkings.floor_id', '=', 'floors.id')
            ->get(["floors.id", "floors.floor_number"]);
    }

    public function parkingsByFloor($id)
    {
        return Parking::where('floor_id', $id)->get(["id", "parking_number"]);
    }
