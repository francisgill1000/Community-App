          ...
          <v-toolbar class="mb-2" dense flat>
            ...
            <span>
              <v-btn
                ...
                class="primary"
                text
                title="Create Room Sub Category"
                @click="addItem"
              >
...
export default {
  ...
  data: () => ({
    ...
    options: {},
    Model: "Room Sub Category",
    endpoint: "room-sub-category",
    search: "",
    snackbar: false,
    ids: [],
    loading: false,
  ...
  async created() {
    ...
    let { data } = await this.$axios.get(`room-category-list`, {
        params: { company_id: this.id },
      });
      this.room_categories = data;
  },
    ...
    submit() {
      ...
      this.$axios
        ...
        .then(({ data }) => {
          ...
          this.errors = [];
          this.snackbar = true;
          this.response = "Room Sub Category inserted successfully";
          this.getDataFromApi();
    ...
    update_data() {
      ...
      this.$axios
        ...
        .then(({ data }) => {
          this.errors = [];
          this.snackbar = true;
          this.response = "Room Sub Category updated successfully";
          this.getDataFromApi();
          this.DialogBox = false;
        })
        .catch(({ response }) => {
