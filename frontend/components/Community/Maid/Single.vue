<template>
  <v-dialog persistent v-model="dialog" width="900">
    ...
    <v-card>
      <v-card-title>View Maid <v-spacer></v-spacer> <v-icon color="primary" @click="dialog = false">mdi-close</v-icon></v-card-title>
      <v-container>
        <v-row>
          <v-col cols="3">
            <v-row>
              <v-col cols="12">
                <div class="text-center">
                  <SnippetsViewPicture :defaultImage="setImagePreview" />
      ...
      <v-divider></v-divider>
      <v-card-title>Associated Tanent(s)</v-card-title>
      <v-container>
        <v-row>
          <v-col cols="12">
            <CommunityMaidAssignTanentList :id="item.id" />
          </v-col>
        </v-row>
...
export default {
  ...
  data: () => ({
    ...
    setImagePreview: "/no-profile-image.jpg",
